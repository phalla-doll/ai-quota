...
- **Staying fresh in a backgrounded PWA** is two mechanisms, and both are needed. Every monitor query in `hooks/use-key-quota.ts` carries `refetchInterval: 60_000`, but query-core gates each tick on `focusManager.isFocused()` (i.e. `document.visibilityState`) unless `refetchIntervalInBackground` is set — so an installed PWA that is buried or on a locked screen polls nothing, and mobile OSes freeze the timer outright anyway. The interval therefore only covers the app while it is on screen; `refetchOnWindowFocus: true` in `components/providers/query-provider.tsx` covers the return trip, refetching whatever went stale while the user was away. Don't "fix" background staleness by turning on `refetchIntervalInBackground` — it burns one Z.ai call per key per minute with the app invisible and still won't fire on iOS. `staleTime` (30s default, 60s on model-usage) is what keeps a quick tab-out/tab-in from stampeding the fan-out.
